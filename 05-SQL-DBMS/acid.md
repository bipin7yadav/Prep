...
2. **Consistency:** A transaction brings the database from one valid state to another, satisfying all declared invariants (`CHECK (balance >= 0)`, foreign keys, uniqueness, and business rules like $\sum \text{debits} = \sum \text{credits}$).
...
### Easy
- **Q:** What does ACID stand for? Define each property with a single sentence.

### Medium
- **Q:** Explain how Write-Ahead Logging (WAL) allows a database to guarantee Durability without immediately flushing entire data pages to disk.
...
- *"What is Group Commit in PostgreSQL/MySQL and how does it prevent disk I/O bottlenecks during peak transaction bursts?"*
  *(Answer: The engine groups multiple concurrent transaction commits into a single batch `fsync()` call, sharing the disk sync latency across dozens of transactions).*
- *"What is the difference between a Local Transaction and a Distributed Transaction across two microservices? Why can't we use standard ACID transactions across microservices?"*

---

## 12. Model Answer: How WAL Guarantees Atomicity & Durability

> **Interviewer:** *"Walk me through the lifecycle of a committed transaction from memory to disk. How is durability preserved if the server immediately loses power?"*
> 
> **Model Answer:**
> "When a transaction executes DML statements, the modifications are made in the **Buffer Pool** in RAM, creating 'dirty pages'.
> 
> Simultaneously, the engine records an append-only log record describing the delta into the **WAL Buffer**.
> 
> When `COMMIT` is called:
> 1. The database does NOT write the dirty 8KB data pages to the table files on disk, because random I/O is slow.
> 2. Instead, it flushes only the WAL buffer to the append-only WAL file on disk using a synchronous OS `fsync()` call.
> 3. Because appending to a log is sequential I/O, it completes in microseconds. Once `fsync` confirms the log bytes are safely on physical disk platter/flash cells, the engine sends a success acknowledgment to the application.
> 
> If the server loses power a millisecond later:
> Even though the actual table files in data storage still contain old balances, the change is 100% durable in the WAL.
> 
> Upon restart, the database runs the **Redo phase** of crash recovery, reads the committed WAL record, and updates the data page in memory and disk, fulfilling the ACID Durability guarantee."
...
- ARIES recovery: Analysis $\to$ Redo $\to$ Undo.
